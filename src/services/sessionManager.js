...
async function checkAndFinalizeInactiveSessions() {
  ...
  try {
    ...
    const activeSessions = await db.allAsync(`
      SELECT c.session_id, c.project, c.owner, MAX(c.timestamp) AS last_activity
...
// ...
    ...
    for (const session of activeSessions) {
      ...
      if (idleTime > INACTIVITY_THRESHOLD_MS) {
        ...
        try {
          await finalizeSession({ sessionId: session.session_id, project: session.project, owner: session.owner });
