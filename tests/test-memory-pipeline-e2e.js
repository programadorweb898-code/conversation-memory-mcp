...
describe('Memory pipeline MCP E2E', function () {
  ...
  beforeEach(async () => {
    ...
    sinon.stub(llmClient, 'generateText').callsFake(async (prompt) => {
      ...
      if (text.includes('JSON AUDIT')) return JSON.stringify({ status: 'missing', reason: 'No existe una memoria durable equivalente.', relatedMemoryIds: [] });
      return JSON.stringify({ candidates: [{ type: 'decision', title: 'Usar PostgreSQL para el historial de conversaciones', what: 'PostgreSQL para almacenar el historial', why: 'Necesitamos persistencia y recuperación del historial', whereContext: 'conversation-memory-mcp', learned: 'La persistencia queda centralizada en Neon', importance: 'high', sourceMessageIds: sourceMessageId ? [sourceMessageId] : [] }] });
    });
    adapter = {
      ...
      promote: sinon.stub().callsFake(async (candidate) => ({ success: true, memoryId: `engram-${candidate.id}`, topicKey: 'postgres-history' })),
    };
  ...
  afterEach(async () => {
    try { if (client) await client.close(); } finally { if (server) await server.close(); sinon.restore(); }
    if (sessionId) {
  ...
  it('recorre finalizeSession -> memoryAudit -> memoryPromote y mantiene la promoción idempotente', async () => {
    const userMessage = await client.callTool({ name: 'saveMessage', arguments: { sessionId, project, role: 'user', content: 'Decidimos usar PostgreSQL para guardar el historial de conversaciones.', agentId: 'e2e-agent' } });
    ...
    await client.callTool({ name: 'saveMessage', arguments: { sessionId, project, role: 'assistant', content: 'Queda registrado como decisión técnica del proyecto.', agentId: 'e2e-agent', relatedMessageId: sourceMessageId } });

    const finalized = parseToolResult(await client.callTool({ name: 'finalizeSession', arguments: { sessionId, project, agentId: 'e2e-agent' } }));
    expect(finalized.summaryGenerated).to.equal(true);
    expect(finalized.auditRequired).to.equal(true);

    const audit = parseToolResult(await client.callTool({ name: 'memoryAudit', arguments: { sessionId, project, agentId: 'e2e-agent' } }));
    ...
    expect(promoted.results).to.deep.equal([{ candidateId, status: 'promoted', memoryId: `engram-${candidateId}` }]);
    expect(adapter.promote.calledOnce).to.equal(true);

    const promotedAgain = parseToolResult(await client.callTool({ name: 'memoryPromote', arguments: { sessionId, project, agentId: 'e2e-agent', candidateIds: [candidateId] } }));
    expect(promotedAgain.results).to.deep.equal([{ candidateId, status: 'already_promoted', memoryId: `engram-${candidateId}` }]);
    expect(adapter.promote.calledOnce).to.equal(true);

    const row = await db.getAsync('SELECT status, promoted_at, engram_id FROM memory_candidates WHERE id = $1', [candidateId]);
